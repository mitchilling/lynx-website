...
> [!NOTE]
> Functions in a module marked as `'shared-runtime'`, while callable by main thread functions, are not automatically identified as main thread functions themselves. If they use main-thread-specific capabilities like `MainThreadRef`, they must still be manually marked with `'main thread'`.
