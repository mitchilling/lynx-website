...
See [Writing Blog Posts](./blog) for details on `<BlogList>` and `<BlogAvatar>`.
...
See [Managing Interactive Examples](./example) for details on `<Go>`.
...
See [Documenting APIs](./api) for details on `<APITable>` and `<APISummary>`.
