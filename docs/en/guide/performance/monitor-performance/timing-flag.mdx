...
2. Get data: register an observer (`PerformanceObserver`) via [`lynx.performance.createObserver()`](/api/lynx-api/lynx/lynx-performance/create-observer) to receive the corresponding performance data (`PipelineEntry`).
