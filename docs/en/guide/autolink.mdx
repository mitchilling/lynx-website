...
Use the Lynx annotations and registration macros in native source files so Autolink can discover the library capabilities. Native Modules usually extend the spec generated by `lynx-autolink-codegen`; Elements and Services are discovered from their native markers.
