---
api: lynx-api/performance-api/performance-entry/reload-bundle-entry
---
...
<img src="https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/plugin/static/reload-bundle-entry-3-7-compressed.png" />
...
The timestamp for the start of initializing the [Background Thread Runtime](/guide/spec#background-thread-runtime) environment. This timestamp is represented as a floating-point Unix timestamp (in milliseconds) with three decimal places. For example: 1739594612307.429.
...
The timestamp for the end of initializing the [Background Thread Runtime](/guide/spec#background-thread-runtime) environment. This timestamp is represented as a floating-point Unix timestamp (in milliseconds) with three decimal places. For example: 1739594612307.429.
...
The time when the container starts preparing the [TemplateBundle](/guide/spec#app-bundle-aka-template-bundle). "Preparation" refers to the container obtaining the TemplateBundle through any means such as network download or loading from local cache. This timestamp is represented as a floating-point Unix timestamp (in milliseconds) with three decimal places. For example: 1739594612307.429.
...
The time when the container finishes preparing the [TemplateBundle](/guide/spec#app-bundle-aka-template-bundle). "Preparation" refers to the container obtaining the TemplateBundle through any means such as network download or loading from local cache. This timestamp is represented as a floating-point Unix timestamp (in milliseconds) with three decimal places. For example: 1739594612307.429.
...
The time when the LynxView starts to be created. This timestamp is represented as a floating-point Unix timestamp (in milliseconds) with three decimal places. For example: 1739594612307.429.
...
The time when the LynxView finishes being created. This timestamp is represented as a floating-point Unix timestamp (in milliseconds) with three decimal places. For example: 1739594612307.429.
...
The time taken from preparing the [TemplateBundle](/api/lynx-native-api/template-bundle) to the completion of the first rendering, with data type [`PerformanceMetric`](/api/lynx-api/performance-api/performance-metric).

Calculation formula: `fcp = ReloadBundleEntry.paintEnd - InitContainerEntry.prepareTemplateStart`.
...
The time taken from loading the [TemplateBundle](/api/lynx-native-api/template-bundle) to the completion of the first rendering, with data type [`PerformanceMetric`](/api/lynx-api/performance-api/performance-metric).

Calculation formula: `lynxFcp = ReloadBundleEntry.paintEnd - ReloadBundleEntry.loadBundleStart`.
...
The time taken from the user opening the page to the completion of the first rendering, with data type [`PerformanceMetric`](/api/lynx-api/performance-api/performance-metric).

Calculation formula: `totalFcp = ReloadBundleEntry.paintEnd - InitContainerEntry.openTime`.
