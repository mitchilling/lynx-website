...
`<scroll-view>` 只具备线性布局的能力，通过线性排列让元件有序呈现。然而面对复杂的界面时，`<list>` 排版选项丰富，可以选择[网格和瀑布流](/api/elements/built-in/list#required-list-type)等不同布局来灵活定制业务需求。
...
- 吸顶能力：`<scroll-view>` [吸顶](/api/elements/built-in/scroll-view.html#吸顶能力)；`<list>` [吸顶](/api/elements/built-in/list.html#实现节点吸顶或者吸底)

- `<list>` [分页滚动](/api/elements/built-in/list.html#利用-item-snap-实现分页滚动)

- `<list>` [下滑加载更多](/api/elements/built-in/list.html#实现滚动到底部加载更多数据)
