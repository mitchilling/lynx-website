...
当开发者只需要监听某个节点的曝光/反曝光事件时，可以通过设置[事件处理器属性](../event-handling/event-propagation.mdx#事件处理器属性)监听节点的 [`uiappear`](../../../api/elements/built-in/view.mdx#uiappear) 和 [`uidisappear`](../../../api/elements/built-in/view.mdx#uidisappear) 事件。
...
- [`lynx.stopExposure [BTS]`](../../../api/lynx-api/lynx/lynx-stop-exposure) 或 [`lynx.stopExposure [MTS]`](../../../api/lynx-api/main-thread/lynx-stop-exposure)：在主线程或后台线程调用，用来停止曝光检测，即不再检测目标节点的可见性，后续不会再触发曝光/反曝光事件。
- [`lynx.resumeExposure [BTS]`](../../../api/lynx-api/lynx/lynx-resume-exposure) 或 [`lynx.resumeExposure [MTS]`](../../../api/lynx-api/main-thread/lynx-resume-exposure)：在主线程或后台线程调用，用来开启曝光检测，即重新开始检测目标节点的可见性，后续正常触发曝光/反曝光事件。
