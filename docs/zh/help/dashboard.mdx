...
在仪表盘顶部，您会找到 **平台选择器**，它允许您在不同平台之间切换以查看其特定的 API 覆盖率。
...
1. **从选择平台开始** - 首先选择您的目标平台以查看相关的覆盖率数据。
2. **使用搜索查找特定 API** - 如果您知道 API 名称，搜索比滚动更快。
3. **查看抽屉获取详情** - 兼容性表格显示特定版本的信息。
4. **监控趋势** - 使用趋势图了解随时间变化的覆盖率改进。
5. **为桌面切换 Clay** - 如果您正在构建跨平台桌面应用程序，请启用 Clay 平台。
