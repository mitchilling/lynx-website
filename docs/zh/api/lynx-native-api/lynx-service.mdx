...
Lynx Service 为 Lynx Engine 提供了宿主特性相关能力，包括图片、日志和 [FetchAPI](/guide/interaction/networking.html#使用-fetch) 相关能力。
...
如果您使用自己实现的 Lynx Service，请删除 Lynx 提供的[默认实现](/guide/start/integrate-with-existing-apps.html?platform=ios)。
...
在[集成文档](/guide/start/integrate-with-existing-apps.html?platform=android)中，将 `LynxServiceCenter` 中的日志服务实现替换为您的实现。
...
在[集成文档](/guide/start/integrate-with-existing-apps.html?platform=harmony)中，将 `LynxServiceCenter` 中的日志服务实现替换为您的实现。
