...
| name       | String    | optional | 动画的名字，可作为动画的唯一标识。该名字会出现在[动画事件](../event/animation-event)参数中，通常用于判断某个动画事件是否是你想要的。                                                                      | 内部的unique ID |
...
Animate API 的事件和 CSS Animation [动画事件](../event/animation-event#animationevent)相同。
